    ...
    public partial class MasterHomePage : ContentPage
    {
        ...
        async void InitLocationListening()
        {
            ...
            if (!LocationHelper.Instance.Geolocator.IsListening)
            {
                try
                {
                    await LocationHelper.Instance.Geolocator.StartListeningAsync(TimeSpan.FromSeconds(10), 1);
                }
                catch (Exception ex)
                {
        ...
        async void PromotionClicked(object sender, EventArgs args)
        {
            var image = sender as Image;
            image.Opacity = 0.6;
        ...
        void RedClicked(object sender, EventArgs args)
        {
            RootPage.ShowSocialMediaPage();
        }
